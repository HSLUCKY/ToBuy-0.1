...
public class RentRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RentRecyclerAdapter(Context context) {
    ...
    public int getItemCount() {
        return ((CommunityActivity) context).rulers[1].size();
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (position == ((CommunityActivity) context).rulers[1].size() - 1) {
            LOAD_DATA = true;
        }

        if (position > 1) {
            if (holder instanceof RentRecyclerAdapter.SimpleCardViewHolder) {
                ...
                instance.getSimpleCardLayout().setType(1);
                /**
                 * 帖子信息初始化
                 */
