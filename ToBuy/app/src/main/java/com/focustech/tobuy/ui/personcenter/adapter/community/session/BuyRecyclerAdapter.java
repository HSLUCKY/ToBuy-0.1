...
public class BuyRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public int getItemCount() {
      /*  if (data == null){
        ...
        return count + 2;
        */
        return ((CommunityActivity) context).rulers[0].size();
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == 0) {
            ...
            guideLayout.getBgLinearLayout().setBackgroundColor(Color.TRANSPARENT);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (position == ((CommunityActivity) context).rulers[0].size() - 1) {
            LOAD_DATA = true;
        }
        if (position > 1) {
            if (holder instanceof BuyRecyclerAdapter.SimpleCardViewHolder) {
                ...
                instance.getSimpleCardLayout().setType(0);
                /**
                 * 帖子信息初始化
                 */
                ...
                /**
                 * 从 position 2 开始   --- > 17   一共15次
                 * 从 0 ， 0 开始   userIndex --- > 0 - > getBaseUserInfos().size()
                 *                  cardIndex -- > 0 - > getCardsLists().get(userIndex).size()
                 *                  正常情况下userIndex 不会越界
                 *                  越界的是cardIndex
                 *
                 *  注意初始化控件和绑定数据是原子操作，不能切分
                 */
