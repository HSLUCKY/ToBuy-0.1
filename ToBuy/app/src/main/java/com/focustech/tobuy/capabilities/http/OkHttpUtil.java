package com.focustech.tobuy.capabilities.http;

import android.os.Handler;

import com.focustech.tobuy.bean.service.base.BaseResp;
import com.focustech.tobuy.capabilities.json.GsonHelper;
import com.focustech.tobuy.capabilities.log.EBLog;
import com.focustech.tobuy.util.GeneralUtils;
...
public class OkHttpUtil {
    ...
    public <T> void requestAsyncPostByTag(String url, String activityName, ITRequestResult<T> iTRequestResult, Class<T> clazz, Param... params) {
        //将Activity和他请求的url封装起来
        addRequestUrl(activityName, url);
        RequestBody body = null;
        if (params.length == 1) {
            MediaType JSON = MediaType.parse("application/json; charset=utf-8");
            body = RequestBody.create(JSON, params[0].value);
        }else {
            FormEncodingBuilder builder = new FormEncodingBuilder();
